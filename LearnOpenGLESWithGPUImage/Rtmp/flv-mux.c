...
static bool build_flv_meta_data(void *context,
		uint8_t **output, size_t *size, size_t a_idx)
{
	...
	*enc++ = AMF_ECMA_ARRAY;
	...
	if (a_idx == 0) {
        ...
        enc_num_val(&enc, end, "width",(double)p_video_encoder->width);
		enc_num_val(&enc, end, "height",(double)p_video_encoder->height);
        ...
        enc_num_val(&enc, end, "videodatarate",(double)p_video_encoder->bps);
		enc_num_val(&enc, end, "framerate",(double)p_video_encoder->fps);
	}
    ...
    enc_num_val(&enc, end, "audiodatarate", (double)p_audio_encoder->bps);
	enc_num_val(&enc, end, "audiosamplerate",(double)p_audio_encoder->samplerate);
	enc_num_val(&enc, end, "audiosamplesize", 16.0);
	//enc_num_val(&enc, end, "audiochannels",(double)audio_output_get_channels(audio));
    enc_num_val(&enc, end, "audiochannels",(double)p_audio_encoder->channels);

	//enc_bool_val(&enc, end, "stereo",audio_output_get_channels(audio) == 2);

    enc_bool_val(&enc, end, "stereo",p_audio_encoder->channels == 2);
    
	//dstr_printf(&encoder_name, "%s (libobs version ",MODULE_NAME);
    dstr_printf(&encoder_name, "%s (libobs version %s","kong","0.0.1");
...
bool flv_meta_data(void *context, uint8_t **output, size_t *size,
