...
static uint64_t all_drop_bytes;
...
{
	if (level > RTMP_LOGDEBUG) //RTMP_LOGWARNING  RTMP_LOGDEBUG
		return;
...
void *rtmp_stream_create()
{
	...
	RTMP_LogSetLevel(RTMP_LOGWARNING); // RTMP_LOGWARNING
    ...
    RTMP_Log(RTMP_LOGINFO, "ian, %s: ok \n", __FUNCTION__);
...
uint64_t os_gettime_ns(void){
    time_t tmp;
    time(&tmp);
    return (uint64_t)tmp*1000000000;
}
...
static void *send_thread(void *data)
{
    ...
    while (os_sem_wait(stream->send_sem) == 0) {
        ...
        int ret = send_packet(stream, &packet, false, packet.track_idx);
		if (ret < 0)
        {
...
static bool send_audio_header(struct rtmp_stream *stream, size_t idx)
{
    ...
    header = p_audio_encoder->header;
    packet.size = p_audio_encoder->headersize;
...
static void send_video_header(struct rtmp_stream *stream)
{
    ...
    header = p_video_encoder->header;
    size = p_video_encoder->headersize;
...
static int try_connect(struct rtmp_stream *stream)
{
    ...
    dstr_catf(&stream->encoder_name, "%d.%d.%d",0,0,1);
/*
...
*/
	...
	/*for (size_t idx = 1;; idx++) {
		//obs_encoder_t *encoder = obs_output_get_audio_encoder(stream->output, idx);
    ...
    {
        //const char *encoder_name = p_audio_encoder->name;
        //RTMP_AddStream(&stream->rtmp, encoder_name);
    }
...
static bool init_connect(struct rtmp_stream *stream)
{
    ...
    dstr_copy(&stream->path,     p_rtmp_service->server_url);
	dstr_copy(&stream->key,      p_rtmp_service->server_key);
	dstr_copy(&stream->username, p_rtmp_service->user_name);
	dstr_copy(&stream->password, p_rtmp_service->user_password);
    ...
    stream->drop_threshold_usec =(int64_t)p_rtmp_service->drop_threshold_ms * 1000;
	stream->max_shutdown_time_sec =(int)p_rtmp_service->max_shutdown_time_sec;
...
static void *connect_thread(void *data)
{
    ...
    if (ret != OBS_OUTPUT_INVALID_STREAM)
...
static void drop_frames(struct rtmp_stream *stream)
{
	...
	while (stream->packets.size) {
		...
		if (packet.type          == OBS_ENCODER_AUDIO ||
            ...
            all_drop_bytes += packet.size;
			num_frames_dropped++;
			obs_free_encoder_packet(&packet);
            stream->drop_next_frame = true;
		}
	}
		...
		struct encoder_packet *packet)
{
    ...
    //前一个非关键帧已丢弃，继续丢弃直到当前帧是关键帧
    if (packet->keyframe)
    {
        stream->drop_next_frame = false;
    }
    else if (stream->drop_next_frame)
    {
        all_drop_bytes += packet->size;
...
{
	struct rtmp_stream *stream = data;
	return RTMP_IsConnected(&(stream->rtmp));
}

//yanyue add丢弃总字节数
uint64_t rtmp_drop_total_bytes()
{
    return all_drop_bytes;
}

//yanyue add获取连接服务器ip（push_ip字段）
char *rtmp_get_ip_addr()
{
    return get_RTMP_Addr();
}
